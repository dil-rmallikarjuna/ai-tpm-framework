...
function callLLM(prompt) {
  ...
  try {
    ...
    // If parsed is a string, keep parsing up to 3 times
    let attempts = 0;
...
async function runTestCase(testCase, steps) {
  ...
  try {
    for (const [i, step] of steps.entries()) {
      ...
      try {
        ...
        pass = false;
        // Take screenshot on failure as well
      }
...
async function main() {
  const filter = process.argv[2]; // e.g., "update_icij_review.txt"
  ...
  for (const tc of testCases) {
    ...
    let steps, result;
    try {
      steps = callLLM(tc.content);
      console.log('LLM steps output:', steps); // Debug log

      // If steps is a string, try to parse it as JSON (handle double-encoded JSON)
      let parseAttempts = 0;
      while (typeof steps === 'string' && parseAttempts < 3) {
        try {
      ...
      result = await runTestCase(tc, steps);
    } catch (err) {
